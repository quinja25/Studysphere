...
```bash
mkdir -p $(dirname .omc/state/team/studysphere-feature-work-2-ind/workers/worker-1/.ready) && touch .omc/state/team/studysphere-feature-work-2-ind/workers/worker-1/.ready
```
...
1. **Claim** your task (run this command first):
   `omc team api claim-task --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"task_id\":\"<id>\",\"worker\":\"worker-1\"}" --json`
...
3. **Send ACK** to the leader:
   `omc team api send-message --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"from_worker\":\"worker-1\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: worker-1 initialized\"}" --json`
...
- **Worker**: worker-1
- **Agent Type**: claude
- **Environment**: OMC_TEAM_WORKER=studysphere-feature-work-2-ind/worker-1
...
- Claim task: `omc team api claim-task --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"task_id\":\"<id>\",\"worker\":\"worker-1\"}" --json`
...
- Release claim (rollback): `omc team api release-task-claim --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"task_id\":\"<id>\",\"claim_token\":\"<claim_token>\",\"worker\":\"worker-1\"}" --json`

## Communication Protocol
- **Inbox**: Read .omc/state/team/studysphere-feature-work-2-ind/workers/worker-1/inbox.md for new instructions
- **Status**: Write to .omc/state/team/studysphere-feature-work-2-ind/workers/worker-1/status.json:
  ```json
  {"state": "idle", "updated_at": "<ISO timestamp>"}
  ```
  States: "idle" | "working" | "blocked" | "done" | "failed"
- **Heartbeat**: Update .omc/state/team/studysphere-feature-work-2-ind/workers/worker-1/heartbeat.json every few minutes:
  ```json
...
- To leader: `omc team api send-message --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"from_worker\":\"worker-1\",\"to_worker\":\"leader-fixed\",\"body\":\"<message>\"}" --json`
- Check mailbox: `omc team api mailbox-list --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"worker\":\"worker-1\"}" --json`
- Mark delivered: `omc team api mailbox-mark-delivered --input "{\"team_name\":\"studysphere-feature-work-2-ind\",\"worker\":\"worker-1\",\"message_id\":\"<id>\"}" --json`
...
1. Write your decision to: .omc/state/team/studysphere-feature-work-2-ind/workers/worker-1/shutdown-ack.json
2. Format:
