"""
Downloads all IB Biology past papers from arrib.qzz.io
Excludes French and Spanish language papers.
Run: python download_biology.py
"""
...
OUTPUT_DIR = r"C:\Users\jaeyo\OneDrive\Desktop\업무용\Biology Past Papers"

YEARS = range(2010, 2026)
SESSIONS = ["May", "November"]
EXCLUDE = ["French", "Spanish"]


def get_biology_paths():
    print("Fetching file list...")
    resp = requests.get(FILE_PATHS_URL, timeout=30)
    resp.raise_for_status()

    paths = [line.strip() for line in resp.text.splitlines() if line.strip()]
    biology = [p for p in paths if "Biology" in p]
    ...
    print(f"Found {len(filtered)} biology files (2010-2025, excluding French/Spanish).")
    return filtered
