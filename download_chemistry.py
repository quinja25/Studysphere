"""
Downloads all IB Chemistry past papers from arrib.qzz.io
Excludes French and Spanish language variants.
Run: python download_chemistry.py
"""
...
def get_chemistry_paths():
    print("Fetching file list...")
    resp = requests.get(FILE_PATHS_URL, timeout=30)
    resp.raise_for_status()

    paths = [line.strip() for line in resp.text.splitlines() if line.strip()]
    chemistry = [p for p in paths if "Chemistry" in p]
    chemistry = [p for p in chemistry if not any(ex in p for ex in EXCLUDE)]
    ...
    print(f"Found {len(filtered)} chemistry files (2010-2025, May & November, English only).")
    return filtered
