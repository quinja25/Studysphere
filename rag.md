...
Past papers live at `past_papers/` in the project root (not tracked by git — local only):

```
past_papers/
  Biology Past Papers/      (558 PDFs — 279 paper groups, 2010–2025)
  Chemistry Past Papers/    (556 PDFs — 280 paper groups, 2010–2025)
  Economics Past Papers/    (383 PDFs — ingested via ingest-past-papers.js)
  Physics Past Papers/      (552 PDFs — 276 paper groups, 2010–2025)
```

Each script defaults to its folder via `path.join(__dirname, '../../past_papers/<Subject> Past Papers')`. Override with `INGEST_DIR` env var.
...
| N | Paper number | `1`, `2`, `3`, `1A`, `1B` |
| TZ | Timezone (optional) | `TZ0`, `TZ1`, `TZ2` |
| Level | Exam level | `HL`, `SL`, `HLSL` |
| `_markscheme` | Suffix for mark scheme files | Present or absent |

**2025 new-syllabus split:** Paper 1 is divided into `1A` (MCQ) and `1B` (structured data questions). `createFilenameParser` handles both. `chunkPaper` routes `1A` → `chunkMCQ`, `1B` → `chunkByQuestions`.
